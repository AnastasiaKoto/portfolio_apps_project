function check ()
{
  ...
  if ((idGroup.length != 0)) {
    idGroup.forEach(function(sheets){
    var doc = SpreadsheetApp.openById(sheets).getSheetByName("итоговый график");
    
      if (doc) {
        this.searchErrors(fileId);
      }
    })
  }
}
...
function searchErrors(fileId) {
  ...
  ss.getSheets().forEach(function(fileId) {
    if (fileId.getSheetName() == "итоговый график") {
      for (let i = 1; i <= fileId.getLastRow(); i++) {
        cell = ['E' + i, fileId.getRange('E' + i).getValue()] ;
        phone.push(cell);
      }
    }
    if (fileId.getSheetName() == "Лист 1") {
      for (let i = 1; i <= fileId.getLastRow(); i++) {
        phone2.push(fileId.getRange('G' + i).getValue());
      }
    }
